//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Jul  5 2017 23:13:50).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "FBPowerLogsDownloadServiceDelegate-Protocol.h"

@class FBDiagnosticsPerformanceMetricSuiteLogger, FBPowerLogsDownloadService, NSString;
@protocol FBDiagnosticsDownloaderDelegate;

@interface FBDiagnosticsDownloader : NSObject <FBPowerLogsDownloadServiceDelegate>
{
    FBPowerLogsDownloadService *_downloadService;
    FBDiagnosticsPerformanceMetricSuiteLogger *_performanceMetricsLogger;
    id <FBDiagnosticsDownloaderDelegate> _delegate;
    unsigned long long _requestedInterval;
}

@property(nonatomic) unsigned long long requestedInterval; // @synthesize requestedInterval=_requestedInterval;
@property(nonatomic) __weak id <FBDiagnosticsDownloaderDelegate> delegate; // @synthesize delegate=_delegate;
- (void).cxx_destruct;
- (void)_collectDataFromSqlLogFiles:(id)arg1 fromDate:(id)arg2 toDate:(id)arg3 completionBlock:(CDUnknownBlockType)arg4;
- (void)_collectAndReturnDiagnosticsDataFromSqlLogFiles:(id)arg1;
- (void)_collectAndLogUnsentDiagnosticsDataFromSqlLogFiles:(id)arg1;
- (void)powerLogDownloadService:(id)arg1 didFinishWithSQLLogFiles:(id)arg2;
- (void)powerLogDownloadService:(id)arg1 didFailWithError:(id)arg2;
- (void)downloadDiagnosticsWithInterval:(unsigned long long)arg1;
- (id)init;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

