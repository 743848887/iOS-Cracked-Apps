//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Jul  5 2017 23:13:50).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@class FBFeedQueryConfiguration, FBGroupCasualGroupsEventListenerAnnouncer, FBGroupColorEventListenerAnnouncer, FBGroupCreationConfig, FBGroupFeedActionHandler, FBGroupFeedCollectionComponentConfiguration, FBGroupFeedCollectionStickyFooterListenerAnnouncer, FBInlineComposerComponentToolboxListenerAnnouncer, FBUserSession, NSArray, NSString;
@protocol FBIntentHandler, FBNavigationCoordinator;

@interface FBGroupFeedCollectionComponentContext : NSObject
{
    NSString *_groupID;
    NSArray *_hoistedStoryIDs;
    long long _source;
    unsigned long long _layoutIdiom;
    FBFeedQueryConfiguration *_queryConfiguration;
    FBGroupCreationConfig *_creationConfig;
    FBGroupFeedActionHandler *_actionHandler;
    FBUserSession *_session;
    FBInlineComposerComponentToolboxListenerAnnouncer *_inlineComposerListenerAnnouncer;
    FBGroupFeedCollectionComponentConfiguration *_configuration;
    id <FBNavigationCoordinator> _navigationCoordinator;
    NSObject *_scenePath;
    id <FBIntentHandler> _intentHandler;
    FBGroupFeedCollectionStickyFooterListenerAnnouncer *_stickyFooterListenerAnnouncer;
    FBGroupCasualGroupsEventListenerAnnouncer *_casualGroupsEventListenerAnnouncer;
    FBGroupColorEventListenerAnnouncer *_colorEventListenerAnnouncer;
}

@property(readonly, nonatomic) FBGroupColorEventListenerAnnouncer *colorEventListenerAnnouncer; // @synthesize colorEventListenerAnnouncer=_colorEventListenerAnnouncer;
@property(readonly, nonatomic) FBGroupCasualGroupsEventListenerAnnouncer *casualGroupsEventListenerAnnouncer; // @synthesize casualGroupsEventListenerAnnouncer=_casualGroupsEventListenerAnnouncer;
@property(readonly, nonatomic) FBGroupFeedCollectionStickyFooterListenerAnnouncer *stickyFooterListenerAnnouncer; // @synthesize stickyFooterListenerAnnouncer=_stickyFooterListenerAnnouncer;
@property(readonly, nonatomic) id <FBIntentHandler> intentHandler; // @synthesize intentHandler=_intentHandler;
@property(readonly, nonatomic) NSObject *scenePath; // @synthesize scenePath=_scenePath;
@property(readonly, nonatomic) id <FBNavigationCoordinator> navigationCoordinator; // @synthesize navigationCoordinator=_navigationCoordinator;
@property(readonly, nonatomic) FBGroupFeedCollectionComponentConfiguration *configuration; // @synthesize configuration=_configuration;
@property(readonly, nonatomic) FBInlineComposerComponentToolboxListenerAnnouncer *inlineComposerListenerAnnouncer; // @synthesize inlineComposerListenerAnnouncer=_inlineComposerListenerAnnouncer;
@property(readonly, nonatomic) FBUserSession *session; // @synthesize session=_session;
@property(readonly, nonatomic) FBGroupFeedActionHandler *actionHandler; // @synthesize actionHandler=_actionHandler;
@property(readonly, nonatomic) FBGroupCreationConfig *creationConfig; // @synthesize creationConfig=_creationConfig;
@property(readonly, nonatomic) FBFeedQueryConfiguration *queryConfiguration; // @synthesize queryConfiguration=_queryConfiguration;
@property(readonly, nonatomic) unsigned long long layoutIdiom; // @synthesize layoutIdiom=_layoutIdiom;
@property(readonly, nonatomic) long long source; // @synthesize source=_source;
@property(readonly, copy, nonatomic) NSArray *hoistedStoryIDs; // @synthesize hoistedStoryIDs=_hoistedStoryIDs;
@property(readonly, copy, nonatomic) NSString *groupID; // @synthesize groupID=_groupID;
- (void).cxx_destruct;
- (id)initWithGroupID:(id)arg1 hoistedStoryIDs:(id)arg2 source:(long long)arg3 layoutIdiom:(unsigned long long)arg4 queryConfiguration:(id)arg5 creationConfig:(id)arg6 actionHandler:(id)arg7 session:(id)arg8 inlineComposerListenerAnnouncer:(id)arg9 configuration:(id)arg10 navigationCoordinator:(id)arg11 scenePath:(id)arg12 intentHandler:(id)arg13 stickyFooterListenerAnnouncer:(id)arg14 casualGroupsEventListenerAnnouncer:(id)arg15 colorEventListenerAnnouncer:(id)arg16;

@end

