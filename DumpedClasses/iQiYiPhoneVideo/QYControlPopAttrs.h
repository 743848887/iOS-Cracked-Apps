//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Jul  5 2017 23:13:50).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <Foundation/NSObject.h>

@class NSString;

@interface QYControlPopAttrs : NSObject
{
    NSString *_priority;
    NSString *_totalTimes;
    NSString *_timesPerDay;
    NSString *_beginTime;
    NSString *_endTime;
    NSString *_showTime;
    NSString *_popRate;
}

+ (id)parseForArray:(id)arg1;
@property(retain, nonatomic) NSString *popRate; // @synthesize popRate=_popRate;
@property(retain, nonatomic) NSString *showTime; // @synthesize showTime=_showTime;
@property(retain, nonatomic) NSString *endTime; // @synthesize endTime=_endTime;
@property(retain, nonatomic) NSString *beginTime; // @synthesize beginTime=_beginTime;
@property(retain, nonatomic) NSString *timesPerDay; // @synthesize timesPerDay=_timesPerDay;
@property(retain, nonatomic) NSString *totalTimes; // @synthesize totalTimes=_totalTimes;
@property(retain, nonatomic) NSString *priority; // @synthesize priority=_priority;
- (void).cxx_destruct;
- (id)initWithCoder:(id)arg1;
- (void)encodeWithCoder:(id)arg1;
- (id)description;
- (void)buildWithDictionary:(id)arg1;
- (id)initWithDictionary:(id)arg1;

@end

