//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Jul  5 2017 23:13:50).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "QYUPhoneCardBaseCell.h"

@class PosterImageView, UIImageView, UILabel;

@interface QYULiveHotHorizontalCell : QYUPhoneCardBaseCell
{
    PosterImageView *_imgView;
    UILabel *_imgLab;
    UIImageView *_imgLabBg;
}

+ (id)getCellObjByContainer:(id)arg1 andIndex:(long long)arg2;
+ (long long)getLinesByCardData:(id)arg1 andShowNum:(long long)arg2;
+ (void)load;
@property(retain, nonatomic) UIImageView *imgLabBg; // @synthesize imgLabBg=_imgLabBg;
@property(retain, nonatomic) UILabel *imgLab; // @synthesize imgLab=_imgLab;
@property(retain, nonatomic) PosterImageView *imgView; // @synthesize imgView=_imgView;
- (void).cxx_destruct;
- (void)imageViewDidClicked;
- (void)rebuildUI;
- (void)setCellModel:(id)arg1;
- (id)initWithStyle:(long long)arg1 reuseIdentifier:(id)arg2;

@end

