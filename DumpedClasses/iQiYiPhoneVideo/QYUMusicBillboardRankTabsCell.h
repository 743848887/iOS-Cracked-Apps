//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Jul  5 2017 23:13:50).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "QYUPhoneCardBaseCell.h"

@class NSMutableArray, UIScrollView, UIView;

@interface QYUMusicBillboardRankTabsCell : QYUPhoneCardBaseCell
{
    UIScrollView *_scrollView;
    UIView *_scrollContentView;
    UIView *_greenLineView;
    UIView *_bottomLineView;
    long long _curSelectedSubTabTag;
    NSMutableArray *_arraySubTabs;
}

+ (id)getCellObjByContainer:(id)arg1 andIndex:(long long)arg2;
+ (long long)getLinesByCardData:(id)arg1 andShowNum:(long long)arg2;
+ (void)load;
- (void).cxx_destruct;
- (void)onSubTabClicked:(id)arg1;
- (void)_refreshButtonSelectStatus;
- (void)setCellModel:(id)arg1;
- (void)layoutSubviews;
- (id)initWithStyle:(long long)arg1 reuseIdentifier:(id)arg2;

@end

